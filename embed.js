(function () {
  ...
  /*────────  STYLES  ────────*/
  const css = `
    .gc{position:fixed;width:60px;height:60px;border:4px solid #3B82F6;border-radius:50%;
        background:rgba(59,130,246,.2);box-shadow:0 0 40px rgba(59,130,246,.9);
        z-index:999999;display:none;align-items:center;justify-content:center;font-size:28px;
    ...
    .gp button{width:100%;padding:10px;background:#3B82F6;color:#fff;border:none;
  ...
  panel.innerHTML = `
     ...
     <div id="run" style="display:none"><div id="info" class="gs"></div></div>
  `;
  ...
  function findElement(keyword){
    ...
    return Array.from(els).find(el => {
      ...
      return [t, ph, dl].some(x => x.includes(keyword)) && el.offsetParent;
    });
  }
  ...
  async function showStep(){
    ...
    $('#info').innerHTML = `<strong>Step ${index+1}/${tutorial.length}</strong><br>${tutorial[index]}`;
    ...
    if (!target){
      $('#info').innerHTML += `<br><em style="color:#f87171">I can’t find “${kw}” – navigate manually.</em>`;
      cursor.style.display = 'none';
      return;
    }
  ...
  $('#go').onclick = async () => {
    ...
    const data = await fetch(API+'/analyze-job', {
      method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({job,industry:$('#ind').value})
    }).then(r=>r.json());

    tutorial = data.tutorial;  keywords = data.keywords;  index = 0;
    await showStep();
  };

  document.addEventListener('click', async ev => {
    if (panel.contains(ev.target)) return;           // ignore UI clicks
    await sleep(500);                                // wait for DOM change
    if (target && (ev.target===target || ev.target.closest('[data-guide-highlight]'))){
      index++; await showStep();                     // correct click
    } else {
      await showStep();                              // re‑hint
    }
  }, true);
